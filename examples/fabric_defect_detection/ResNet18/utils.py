...
def random_train_generator(train_path, save_path, num=3000):
    tg = TGData(train_path)
    aug = ImageRandomDistort()
    
    file_list = gb.glob(train_path+r"/*.bmp")
    ...
    while True:
        ...
        for file in file_list:
            ...
            if label == 1: img_file = os.path.join(save_path, str(label)+"_"+str(i)+".png")
            else: img_file = os.path.join(save_path, str(label)+"_"+str(i)+".png")
...
def random_valid_generator(valid_path, save_path_pos, save_path_neg, num=1000):
    tg = TGData(valid_path)
    aug = ImageRandomDistort()
    
    file_list = gb.glob(valid_path+r"/*.bmp")
    file_list = shuffle(file_list)
    ...
    while True:
        ...
        for file in file_list:
            ...
            fname, _ = os.path.splitext(filename)
            box, label = tg.generate(fname)
            img_file = os.path.join(valid_path, fname+".bmp")
...
if __name__ == "__main__":
    ...
    save_path = r"E:\Projects\Fabric_Defect_Detection\model_proto\dataset\ThreeGun\pos_neg_train"
    random_train_generator(train_path, save_path, num=3000)
    
    # save_path_pos = r"E:\Projects\Fabric_Defect_Detection\model_proto\dataset\ThreeGun\pos_valid"
    # save_path_neg = r"E:\Projects\Fabric_Defect_Detection\model_proto\dataset\ThreeGun\neg_valid"
    # random_valid_generator(valid_path, save_path_pos, save_path_neg, num=1000)
